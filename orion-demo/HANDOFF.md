...
- 当前状态：线上为 `express-knlw-049`（2026-06-12 修复绑定申请微信登录身份显示、核验信息口径和小程序发起接龙权限门控）。小程序 AppID `wx8ad6ccfa1b8f040a` 已通过 `miniprogram-ci` 上传开发版本 `1.0.3`，robot `1`，备注为“2026-06-13 底部tabBar驾驶舱首页改版与UI三态防误修复”（前一版 `1.0.2` 为绑定申请显示与接龙权限）；尚未提交微信审核或发布正式版。后端/网页端 `express-knlw-049` 已通过 `deploy:verify`，线上 `/api/health` 200 且 DB OK。
...
- 当前线上版本：`express-knlw-049`，2026-06-12 部署并验证（绑定申请微信登录身份显示 + 核验信息口径 + 发起接龙权限门控 + DB OK）。
- 当前小程序开发版本：`1.0.2`，robot `1`，已上传到微信后台；尚未提交审核或发布正式版。
...
小程序已上传内容摘要（当前开发版 `1.0.2`；尚未提交审核或发布正式版）：

- `miniprogram/` 微信小程序工程、`project.config.json`、小程序调用配置与页面源码；比赛记录页已做中文友好现场操作，保存到现有 `/api/games` JSON 字段；当前开发版 `1.0.2` 已上传微信后台。
...
- 小程序管理台：`pages/admin/admin` 已注册到 `app.json` 和开发者工具条件编译；首页登录状态和个人面板会对有权限用户显示管理入口。当前管理台覆盖移动端高频操作：读取 `/admin/bind-requests?status=pending` 做绑定审批，调用 `/admin/bind-invitation-options` 加载可邀请用户和正式球员，调用 `/admin/bind-invitations` 发送绑定码站内信邀请，A 级账号可读取 `/admin/users` 并调用 `/admin/users/:id/admin-level` 调整普通/C/B/A 与数据组/运营组权限，也可调用 `/admin/users/:id/app-connect-code` 为小程序先注册用户生成 30 分钟有效的网页关联码并自动复制，调用 `/admin/users/:id/reset-password` 重置邮箱网页登录密码，调用 `/admin/users/:id/bind-player` / `/admin/users/:id/unbind-player` 直接修正账号和正式球员档案绑定，输入“删除账号”确认后调用 `DELETE /admin/users/:id` 删除账号；有 `system:settings` 权限者可读取 `/site-settings/players-starfield`，用移动端预设/核心参数发布球员页全站星阵配置；有 `events:write` 权限者可在管理台勾选近期接龙，输入“删除接龙”并二次确认后逐条调用 `DELETE /api/events/:id` 批量删除接龙；有 `players:write` 权限者可调用 `POST /api/players` 新增试训/正式球员，也可调用 `POST /api/players/merge` 合并重复档案；小程序球员详情管理区已能维护同源 `players.photo`，支持粘贴 COS URL、选择图片上传到 COS（`kind = player`）和清空照片，继续与公开展示头像 `players.public_avatar` 分离；有 `tournaments:write` 权限者可读取 `/tournaments` 并调用 `POST /api/tournaments` / `PATCH /api/tournaments/:id` 创建或编辑赛事容器，有 `destructive:delete` 权限者选中既有赛事并输入“删除赛事”确认后可调用 `DELETE /api/tournaments/:id` 删除赛事容器；有 `games:revise` 权限者可读取 `/games?includeAggregate=false`，按全部/未关联/指定赛事筛选比赛，勾选后调用 `PATCH /api/games/batch-reassign` 批量移动到目标赛事，后端同步 `tournament_id / season / season_name` 并写 `batch_reassign_games` 审计；同时有 `destructive:delete` 权限者可输入“删除比赛”并二次确认后逐场调用 `DELETE /api/games/:id` 批量删除选中比赛，后端逐场写 `delete_game` 审计；有 `games:draft` 且有 `games:confirm` 权限者可在小程序数据导入页一次选择多份聊天 PDF / Excel（XLS / XLSX），逐份或逐场调用 `/games/import-gamechanger` 解析预览，可按队列项上传可选封面图到 COS，并用 `/games` 确认入库；比赛详情页会对有 `games:revise` 权限者显示“修订比赛数据”，进入 `pages/games/game-edit/game-edit` 后可修订赛事归属、日期、封面图、场地、主客队、比分、R/H/E、MVP、备注、本队打击行和投手行，封面图可粘贴 COS URL 或重新上传到 COS，MVP 支持从本队行选择并保存 `mvpPlayerId`，保存时必须填写修订原因并调用 `PATCH /api/games/:id`，后端继续写 `revise_game_data` 审计；有 `destructive:delete` 权限者可在同一修订页输入“删除比赛”后删除单场比赛，后端写 `delete_game` 审计；读取 `/leaderboard` 展示网页后台同款全员积分总览、总分和出场/表现/奖项/手动拆分并可点回球员详情，调用 `/points-adjustments` 做手动加减分并可读取/删除选中球员近期手动积分，调用 `/attendances` 补录训练/活动签到并可读取/删除选中球员近期签到，调用 `/hall-of-fame` 授予/移出名人堂，读取 `/admin/audit-logs?limit=20` 展示最近审计摘要，统计待审精彩时刻，并提供发起接龙、新建比赛、球员阵容、名人堂、精彩时刻、站内通知和审计日志快捷入口。`pages/admin/audit/audit` 支持按对象、动作和关键词筛选审计日志，加载更多，查看并复制 metadata 明细；`/admin/audit-logs` 支持 `limit/offset/targetType/action/q` 参数。数据组只有 `audit:game_read` 时后端只返回比赛相关审计，A/B 管理员有 `audit:read` 时可看全站审计。`pages/notifications/notifications` 对普通队员展示站内信列表和未读数；有 `notifications:write` 权限的管理员可调用 `POST /api/notifications` 按全队、正式球员、试训队员、管理员或指定球员群发站内信，后端写 `user_notifications` 并记录 audit。
- 比赛发起页的出场名单支持两种来源：`管理员手动添加` 和 `从活动接龙导入`。球员候选通过 `/players?include=all&limit=50&offset=...&keyword=...` 分页搜索，接龙导入读取 `/api/event-signups?eventId=...`，只把已绑定 `playerId` 的报名/待定队员直接导入 lineup；如果绑定球员不在当前候选页，会单独读取 `/players/:id` 补入，避免误判为未绑定。从微信群粘贴导入但未匹配档案的 `manualName` 会列为“需手动添加”，不能直接成为统计球员。管理员仍可继续勾选/取消球员并调整棒次、守位。
...
- 小程序比赛记录新增 `eventId` / `games.event_id`、`mvpPlayerId` / `games.mvp_player_id`、`gameLog` / `games.game_log` 和 `games.metadata`：现场记录页维护上/下半局、出局数和垒位，能识别的垒上跑者用 `playerId/name` 保留；1B/2B/3B/HR/BB 会按常规跑垒自动推进垒位、写入比分，并在能带回分数时同步得分跑者 R 和打者 RBI，日志保存 `scoredRunners`；打席结果会自动轮到下一棒，记录员也可以通过 picker 或点击本队数据表直接切换当前打者；现场阵容调整区可记录换守位、换人、代打/代跑等备注，更新 batting row 的 `pos` 并写入事件日志/保存 payload；手动得分、手动打点、垒位开关和撤销仍保留用于特殊规则或赛后修正。发起页日期使用小程序日期 picker，默认当天；不再预先填写局数，现场记录页从第 1 局开始，点“下一局 / 换半局”时按实际记录动态扩展逐局比分，保存时用实际 linescore 长度写入 `innings`。得分、打席、投手、阵容调整和对手统计会生成中文事件日志；涉及猎户球员的日志会同时保存 `gameId`、`playerId`、`playerName`、`actionKey/actionType`、`scoredRunners` 和 `relatedEventId`，比赛详情页可从逐事件日志点回球员详情，并通过顶层 `eventId` 与 metadata 回溯接龙活动、赛事/场次来源、名单来源、MVP 和出场球员 id。`server/routes/games.js` 在保存/修订时会懒加载补 `event_id` / `mvp_player_id` / `game_log` / `metadata` 列，支持 `/games?eventId=...` 按来源接龙筛选，并为能匹配到球员池的猎户侧打击/投手行补写 `playerId`。
...
- 本文件和 `../DESIGN_BRIEF.md` 已记录当前状态；线上网页端和 Express API 为微信云托管 `express-knlw-049`，小程序开发版本 `1.0.2` 已上传但尚未提交审核或发布正式版。当前运行时代码已部署/上传；文档同步改动本身不影响运行包，工作区仍有未提交改动，用户说“备份代码”时执行 commit + push。
...
4. 小程序线：首版源码和配套 API 已完成并已上传开发版本 `1.0.2`；活动签到为一键定位签到（当前调用 `wx.getLocation({ type: 'gcj02' })`），比赛记录为中文现场操作页，并已加入常见打席结果自动跑垒/RBI/比分联动；微信开发者工具 Stable ARM64 `2.01.2510290` 已安装到 `/Applications/wechatwebdevtools.app`，CLI 位于 `/Applications/wechatwebdevtools.app/Contents/MacOS/cli`。后续提交审核、发布正式版、再次上传开发版本或真机预览都需要用户明确授权。资源复用已关联到 `prod-d5gtkxdyu7263e95b` 并在模拟器读取到首页云数据；云托管 `express-knlw-049` 已配置小程序 AppID/AppSecret，假 code 登录返回微信 `invalid code` 属预期。具体方案见 §10。
