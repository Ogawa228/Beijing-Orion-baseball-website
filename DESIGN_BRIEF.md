...
最后整理：2026-06-13
当前线上：`express-knlw-055`

Claude Design / Claude Code 接手摘要（2026-05-29）：

- 小程序视觉目标：继续和猎户座官网保持同一套深蓝、金色、星阵、圆形队徽与公开身份/磨砂隐私语义；这是队内工具，不做营销式 landing page。
- 当前产品状态：网页和云托管后端已部署到 `express-knlw-055`；小程序已上传微信后台开发版本 `1.0.11`（robot `1`），但尚未提交审核或发布正式版。小程序已覆盖赛事活动统一工作台、赛事容器创建/编辑/归属整理、活动接龙、报名、管理员粘贴微信群接龙识别、`wx.chooseLocation` 接龙选点、日期/时间 picker、`wx.getLocation` 精确定位签到、微信登录后直接申请绑定正式球员档案、球员阵容/详情、积分榜、精彩时刻照片墙、比赛工作台“开始记录比赛”、比赛发起三步流程、拖拽/点选棒次守位、中文现场记分台、慢投垒 `1-1` 和两好界外三振规则、比赛详情和 PDF 导出、精简管理台、站内通知，以及 Apple 式圆角/居中按钮体系。
- 最新设计/性能决策：移动端所有大候选列表都必须分页或后端 keyword 搜索，不能为了 picker、摘要或 meta 拉全量球员/比赛/赛事/精彩时刻。管理台积分关联比赛搜索只展示 `/games?includeAggregate=false&keyword=<关键词>&limit=40&offset=0` 的小候选，且不得影响批量移动/删除列表。小程序按钮保持“品牌内 Apple 化”：深蓝/金色不变，操作按钮使用 flex 居中、22-26rpx 圆角或胶囊圆角、稳定 hover/disabled，禁止回退到方角或依赖 `line-height` 顶偏的按钮。
- 文案要求：面向中文队员，分类标签使用中文 + emoji，不回退到英文 chip；“高光图片”统一为“精彩时刻”；B站只提供复制/跳转语义，不在小程序内做外链播放器。
- 验证状态：小程序上传前完整 `npm run test:miniprogram-ci`、`npm run test:miniprogram-flows`、`npm run test:miniprogram-preflight`、相关 `node --check` 与 `git diff --check` 通过；微信后台开发版本 `1.0.11` 上传成功；云托管 `express-knlw-055` 已通过 `deploy:verify`，线上 `/api/health` 200 且 DB OK。注意同轮 `express-knlw-054` 构建失败未切流，随后重试 `express-knlw-055` 成功。后续视觉改动也要同步本文件与 `orion-demo/HANDOFF.md`。

2026-06-14（十二）图片上传统一高可用工具（小程序开发版 `1.0.14`；后端无改动仍 `express-knlw-057`）:① 所有图片上传统一走 `miniprogram/utils/upload.js` 的 `uploadImage(file, kind, prefix)`,不再各页面复制上传代码。② 质量对齐网页端:`wx.compressImage` 限宽 1280 + 质量 78(网页端为 1280/q0.78)。③ 高可用双通道:主通道 `wx.uploadFile` multipart 直传 `/api/upload`(绕过 callContainer 大小限制),失败兜底 `callContainer` base64 传 `/api/upload/base64`,杜绝 cloud callContainer 大图上传 BUG。④ 启用主通道需在微信后台配 uploadFile 合法域名(云托管服务域名);未配自动降级兜底,功能不受影响。验证:`npm run test:miniprogram-ci` 17 项全绿。

2026-06-14（十一）赛事封面上传与地点纯地图选点口径（已部署 `express-knlw-057` / 小程序开发版 `1.0.13`）:① 小程序图片上传走云托管 `callContainer`,请求体有大小限制,所有图片上传前必须 `wx.compressImage`(限宽 1280 + 质量 65,带旧基础库 fallback)压缩,避免大图 base64 触发 callContainer 失败;赛事封面、活动封面/配图已加压缩(共享 readFileBase64),profile/admin 头像图小暂未加。② 地点选择一律纯地图选点 `wx.chooseLocation`,不再保留文字输入框——赛事管理页地点改为只读展示 + 点击触发选点(与签到地理围栏需要坐标的设计一致)。验证:`npm run test:miniprogram-ci` 17 项全绿、`deploy:verify --expected-version express-knlw-057` 全通过。

2026-06-13（十）签到地理围栏与地点选点口径（已部署 `express-knlw-056` / 小程序开发版 `1.0.12`）:① 选择地点统一走 `wx.chooseLocation` 地图选点——活动接龙与赛事管理页一致,不再纯手动输入文本。② 签到地理围栏:接龙用地图选点设了坐标(`metadata.location.latitude/longitude`)时,签到位置必须在 **500 米**内,后端 Haversine 权威校验(超出 403 `too_far` 带实际距离、缺定位 400 `location_required`);无坐标的老接龙放行。`wx.getLocation` 与 `wx.chooseLocation` 同为 GCJ-02 坐标系可直接算距离。③ tabBar 列表页数据变更后刷新口径:赛事/接龙的创建、编辑、删除成功后写 `orionEventHubDirty` storage 标志,返回 `event-list` 时 onShow 检测则刷新一次并清标志,平时不刷新以免切 tab 频闪。验证:`npm run test:miniprogram-ci` 17 项全绿(新增 `test:checkin-geofence`)、本地起服务实测创建赛事 201 与 Haversine 距离、`deploy:verify --expected-version express-knlw-056` 全通过。

2026-06-13（九）小程序 Apple 风格按钮与 UI 居中优化（已部署后端/网页端 `express-knlw-055`；已上传开发版本 `1.0.11`,robot `1`；尚未提交审核或发布）:① 保留猎户深蓝/金色品牌，不做 Liquid Glass 大改；全局 `button` 和 `.btn` 改为 flex 水平/垂直居中，主按钮约 24rpx 圆角，小按钮约 20rpx，筛选/标签/分段控件用胶囊圆角，并补稳定 hover/disabled 状态。② 赛事活动页三张主操作卡改为居中 action tile，“接龙 / 赛事 / 比赛”改为胶囊式 segmented control。③ 管理台顶部模块 tabs、球员档案分段、待办卡、快捷入口和 `link/tiny-link` 等实际操作控件统一圆角和居中，只保留真正文本链接为链接视觉。④ 现场记分台高频打席按钮、更多结果、模式切换、撤销/垒位按钮，以及比赛中心、发起接龙、接龙详情、赛事管理页、个人页退出登录等按钮统一尺寸、圆角和文本居中。⑤ `test:miniprogram-preflight` 增加按钮体系守卫，阻止 `button { border-radius: 0 }`、主按钮圆角不足、按钮不居中等回归。验证：`node --check scripts/verify-miniprogram-preflight.js`、服务端 JS `node --check`、本地生产模式启动和 `/api/health`、完整 `npm run test:miniprogram-ci`、`npm run test:miniprogram-flows`、`npm run test:miniprogram-preflight`、`git diff --check`、`miniprogram-ci upload`、`wxcloud run:deploy` 发布 `express-knlw-055` 和 `deploy:verify --expected-version express-knlw-055` 通过，线上 `/api/health` 200 且 DB OK。

2026-06-13（八）小程序赛事活动、赛事管理、时间选择与管理台 UI 优化（已部署后端/网页端 `express-knlw-053`；已上传开发版本 `1.0.10`,robot `1`；尚未提交审核或发布）:① 底部 tab 排布保持 `赛事活动 / 数据中心 / 签到 / 积分榜 / 我的`，但 `pages/events/event-list` 升级为统一工作台：顶部三张主操作卡为“开始记录比赛 / 新建赛事 / 发起活动接龙”，下方分段为“接龙 / 赛事 / 比赛”，避免普通用户在活动、赛事、比赛之间来回找入口。② 新增 `pages/tournaments/tournament-manage`，赛事容器的创建、编辑、封面、简介和比赛归属整理全部从赛事活动进入；卡片只放“详情 / 编辑”，删除收进编辑页并要求输入“删除赛事”。③ 所有需要确定日期或时刻的字段使用小程序 picker，默认当天；具体时刻拆成日期 picker + 时间 picker，旧 `YYYY-MM-DD HH:mm` 自动解析回点选状态。④ 管理台作为跨模块后台，不再藏赛事 CRUD；顶部待办摘要引导到绑定审批、待审精彩时刻、近期接龙和未关联比赛，球员档案内部分成“球员池 / 新增球员 / 批量导入 / 合并档案”。⑤ 发起接龙删除小红书/帖子识别，只保留中性的外部链接字段；接龙详情保留“粘贴并识别群接龙 / 识别输入框内容”，识别结果用“已识别 / 已匹配 / 待人工确认”给管理员手动矫正。验证：完整 `npm run test:miniprogram-ci`、`npm run test:miniprogram-flows`、`npm run test:miniprogram-preflight`、`npm run test:tournaments-audit`、相关 `node --check`、`git diff --check`、`miniprogram-ci upload`、`wxcloud run:deploy` 发布 `express-knlw-053` 和 `deploy:verify --expected-version express-knlw-053` 通过，线上 `/api/health` 200 且 DB OK。

2026-06-13（七）小程序功能 UI 与慢投垒记分优化（已部署后端/网页端 `express-knlw-052`；已上传开发版本 `1.0.9`,robot `1`；尚未提交审核或发布）:① 底部 tab 排布保持 `赛事活动 / 数据中心 / 签到 / 积分榜 / 我的`，但 `pages/events/event-list` 改成“赛事活动工作台”，首屏只保留“开始记录比赛 / 查看比赛记录 / 发起活动接龙”，下方分区展示最近接龙和最近比赛。② `pages/games/game-list` 改成“比赛工作台”，`开始记录比赛` 是唯一主按钮，`导入 GameChanger`、筛选比赛、赛事赛季作为次级操作；比赛卡片继续突出对阵、比分、日期和赛事。③ `pages/score/create` 改成“比赛信息 -> 出场名单 -> 守位确认”三步流程，接龙导入后用“已识别 / 已绑定 / 需手动添加”说明名单完整度，守位排布保留场地图并强化点选守位入口。④ `pages/score/live` 改成“现场记分台”：顶部比分条固定展示比分、局数、出局、垒位、当前打者、球数和撤销；主屏高频按钮为坏球、好球、记录界外球、安打、出局、保送/上垒和更多结果；底部模式切换为“打席 / 跑垒 / 投手 / 阵容 / 日志 / 赛后”。⑤ 慢投垒 `softball` 初始和新打席都是 `1-1`，界外球算好球，两好界外自动三振；棒球 `baseball` 保持 `0-0` 且两好界外不三振；保存 payload 新增 `metadata.scoreRules` 和逐日志球数/垒位/结果字段，比赛详情展示规则、球数和垒位推进。⑥ `.tab-page` 底部避让更新为 `220rpx`，个人页退出登录保持全宽危险操作按钮。验证：完整 `npm run test:miniprogram-ci`、相关 `node --check`、`git diff --check`、`miniprogram-ci upload`、`wxcloud run:deploy` 发布 `express-knlw-052` 和 `deploy:verify --expected-version express-knlw-052` 通过，线上 `/api/health` 200 且 DB OK。

2026-06-13（六）小程序“赛事活动”入口、底部遮挡修复与云托管全量部署（已部署后端/网页端 `express-knlw-051`；已上传开发版本 `1.0.8`,robot `1`；尚未提交审核或发布）:① 底部左侧 tab 改名为“赛事活动”，同时承载活动和比赛入口；首页“近期活动”右侧链接同步为“赛事活动”。② `pages/events/event-list` 顶部新增 hub 操作区，运营/记录权限用户可直接点“开始记录比赛”，旁边保留“比赛中心 / 发起接龙 / 刷新”；按钮用两列 grid，主按钮跨两列，保证窄屏比例稳定。③ 5 个 tab 页统一 `.tab-page` 安全滚动容器，把页面滚动区域控制在固定 tabBar 之上，解决内容和底部 tabBar 重叠。④ 个人页“退出登录”改为全宽红色危险操作按钮，高 96rpx。⑤ 按用户“全部部署”要求补做云托管全量发布。验证：完整 `npm run test:miniprogram-ci`、相关 `node --check`、`git diff --check`、`miniprogram-ci upload`、`wxcloud run:deploy` 发布 `express-knlw-051` 和 `deploy:verify --expected-version express-knlw-051` 通过。

2026-06-13（五）小程序管理台精简、绑定申请制与比赛中心记分入口（已部署后端 `express-knlw-050`；已上传开发版本 `1.0.7`,robot `1`；尚未提交审核或发布）:① 比赛记录主入口保留在比赛中心，按钮文案统一为“开始记录比赛”；活动中心列表只保留“发起接龙 / 刷新”，活动详情可继续带接龙上下文进入记分。② 小程序绑定统一走申请审批，不再展示绑定码、网页关联码、直接绑定/解绑和账号删除等旧入口；审计页将相关旧 action 作为历史操作显示。③ 管理台首页改为模块 tabs，保留绑定审批、球员档案、积分签到、账号权限、最近操作等高频能力，移除星阵发布、赛事设置、接龙/比赛批量删除、名人堂授予等低频或高风险面板入口。④ 全局 `.page` 底部避让改为纯 `240rpx`，避免混合 `rpx + env(px)` 在部分机型失效导致内容压到底部 tabBar 下。验证：完整 `npm run test:miniprogram-ci`、相关 `node --check`、`git diff --check`、`deploy:verify` 和线上 `/api/health` 通过。
...
部署状态：当前线上为 2026-06-13 全量部署验证通过的 `express-knlw-055`（Apple 风格按钮与 UI 居中优化 + 小程序 `1.0.11` 同步全量部署 + DB OK），小程序微信后台开发版本为 `1.0.11`，尚未提交审核或发布正式版。2026-05-26/27 本地新增微信小程序工程；小程序请求层支持官方云托管已关联环境调用，也支持 `wx8ad6ccfa1b8f040a` 通过 `resourceAppid: wx7dce60930ee10898` / `resourceEnv: prod-d5gtkxdyu7263e95b` 跨环境访问同一套 Express API + MySQL，不另建孤岛数据源，请求头带 `X-WX-SERVICE: express-knlw`。2026-05-27 资源复用授权后，开发者工具模拟器已确认小程序首页可读取同一套云托管数据：`/auth/me`、`/events`、`/games?includeAggregate=false`、`/leaderboard` 均 200，积分榜和最近比赛正常渲染；原 `-601012 unauthorized env / 【资源复用】未获得该环境授权` blocker 已解除。编译面板剩余 `Error: timeout` 定位为默认云实例初始化与跨资源实例混用导致的开发工具运行时超时风险，已调整为只有直连本小程序云环境或显式设置 `cloudInitEnv` 时才执行默认 `wx.cloud.init()`。最新验证已通过 `npm run test:miniprogram-ci`、`npm run test:miniprogram-flows`、`npm run test:miniprogram-preflight`、`npm run test:tournaments-audit`、`node --check`、`git diff --check` 和 `deploy:verify --expected-version express-knlw-055`。小程序首页保留 `球员阵容` 入口，与网页端同源展示 `/players` + `/leaderboard` 的正式球员、背号、守位、公开展示名/头像、积分和搜索筛选；球员详情页继续串联 `/players/:id`、`/players/:id/points` 和 `/games`，展示档案、积分拆分、最近积分流水和相关比赛摘要；个人页可维护账号昵称/头像，并允许已绑定正式球员维护球员页公开展示名称/公开头像，继续保持账号资料、真实档案和公开展示资料分离；小程序新增 `pages/hall-of-fame/hall-of-fame` 名人堂页，并在首页下方荣誉区展示最近入选，不占用首屏五个主流程卡片；小程序新增 `pages/highlights/highlights` 精彩时刻页，复用 `/highlights`、`/players`、`/games`，支持图片投稿、图片预览、B站链接复制和管理员发布/退回待审图片，首页下方展示已发布图片但不改变首屏五个主流程；比赛中心同源读取 `/games` + `/tournaments`，支持全部/慢垒/棒球和赛事/赛季筛选，老比赛无 `tournamentId` 时用 `season` 兜底匹配；比赛详情页的猎户打击/投手行和 MVP 会匹配 `/players`，用公开展示名显示并可点回球员详情。队内接龙支持小程序卡片转发、管理员 `wx.chooseLocation` 地图选点和管理员粘贴微信群接龙文本识别导入；签到为一键定位（当前权限路线调用 `wx.getLocation` 精确定位），签到成功页显示本次积分变化、当前总积分和升级提示，积分页展示个人积分拆分和最近流水。小程序已有 `pages/admin/admin` 精简管理台，按模块 tabs 保留绑定审批、球员档案、积分签到、账号权限和最近操作；不再展示绑定码、网页关联码、直接绑定/解绑、账号删除、星阵发布、赛事设置、接龙/比赛批量删除和名人堂授予等旧入口。比赛发起页支持从活动接龙导入已绑定球员出场名单，也允许管理员继续手动补人、删人；棒次和守位以可拖动球员块为主，picker 兜底，拖到场地图会吸附到最近守位。比赛记录为中文现场操作页，覆盖逐局、上/下半局、出局数、垒位、常见打席结果自动推进垒位/RBI/比分、打席后自动轮到下一棒、直接选择当前打者、现场阵容调整、事件日志、打线、投手、对手统计、MVP、备注和撤销；保存后详情页按 box score 回看 R/H/E、长打、守位、投手细项、对手统计和逐事件日志，MVP 和可识别球员均可回到球员档案。微信登录后未绑定正式球员档案的用户会直接进入绑定申请页；绑定申请页明确展示选中的正式球员，申请中携带目标球员 ID 和球衣号供管理员核验；个人页和绑定页都显示绑定申请审核状态。
...
- 小程序管理台是移动端高频操作面板，不是网页 `admin.html` 的完整复刻：管理员/数据组/运营组可从首页或个人面板进入，顶部用模块 tabs 分为绑定审批、球员档案、积分签到、账号权限和最近操作；当前只保留移动端必要能力，包括绑定申请审批、球员新增/批量导入/合并、账号权限调整、手动积分调整、删除错误手动积分、补录训练/活动签到、删除错误签到、精彩时刻待审统计、最近审计摘要、站内通知和常用跳转。小程序端不再展示主动发送绑定邀请、绑定码中心、网页关联码、直接绑定/解绑、账号删除、星阵全站发布、赛事设置、接龙/比赛批量删除和名人堂授予等旧入口；绑定统一走申请审批，旧审计 action 只作为历史记录展示。比赛数据修订和单场删除仍从比赛详情/修订页进入，比分、球员行、MVP 或封面错误优先修订，不靠删除重建绕过审计。`pages/admin/audit/audit` 读取 `/admin/audit-logs`，支持 `limit/offset/targetType/action/q` 分页筛选、查看和复制 metadata 明细；数据组只有 `audit:game_read` 时只看比赛审计，A/B 管理员有 `audit:read` 时可看全站审计。站内通知页普通队员看未读/已读列表；有 `notifications:write` 权限的管理员可按全队、正式球员、试训队员、管理员或指定球员发送站内信，后端写 `user_notifications` 并记录 audit。
...
- 球员池：正式/试训、合并、升级、编辑。
- 用户账户：绑定申请审批、角色、重置密码。
- 绑定方式：小程序端走申请审批；绑定码/关联码仅作为网页端或历史兼容能力，不再放入小程序管理台。
- 积分管理：手动加减分、训练/活动签到、全员总览。
- Match：赛事设置 / 比赛数据 / 数据确认 / 数据修订。

权限模型采用“等级 + 权限包 + 权限点”，不要只依赖 `role='admin'`：

- A 全站级 / Owner：默认仅 `admin@orion.cn`。拥有全部权限，可授予/回收 B/C 权限、重置密码、查看全部审计日志、执行高风险删除、修改系统设置；网页端保留的历史绑定/关联工具不进入小程序管理台。球员页群星排列、`site_settings/players-starfield`、全站视觉配置只属于 A。
- B 队长级：包含数据组和运营组两个同级权限包，并额外拥有球队身份和成绩事实权限：审批绑定申请、合并/升级/编辑球员、修改号码/位置/别名/正式状态、授予荣誉 title、管理名人堂、记录签到、调整积分。
...
- 网页端不能自动通过绑定申请，因为邮箱注册缺少微信 openid 这样的身份上下文；小程序端统一走“申请绑定 -> 管理员审批”，不再展示绑定码或站内绑定邀请入口。
