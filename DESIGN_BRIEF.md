...
最后整理：2026-05-21  
当前线上：`express-knlw-034-20260521133747`

部署状态：2026-05-21 13:40 已部署 `express-knlw-034-20260521133747`：球员页动效性能优化、注册入口合规增强、公开身份/真实身份磨砂化、轻量玻璃闪烁、磨砂头像边缘对齐修正和离屏/隐藏态动画节流均已上线。部署后仅本文档和 `orion-demo/HANDOFF.md` 有记录性更新。
...
| `players.html` | 球员墙 | 首屏星阵头像节点、题字“猎户群星闪耀时”、点击节点弹 v2.4 球星卡、管理员星阵编队台；`?wallpaper=1` 可作为 Wallpaper Engine 动态壁纸源 | 星阵优先；非正式用户优先看公开名/公开头像，未设置时看磨砂玻璃化档案名/档案照，并有低成本玻璃高光闪烁；球星卡只在点击后 modal 出现；壁纸模式隐藏操作 UI |
...
- 首页 footer 悬挂 `Edited by 江山` 和 ICP 备案号，排布为一行 meta 信息：版权 / 署名 / 备案号，备案号链接到工信部备案官网 `https://beian.miit.gov.cn/`。
