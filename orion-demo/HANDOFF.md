...
- 当前状态：线上为 `express-knlw-049`（2026-06-12 修复绑定申请微信登录身份显示、核验信息口径和小程序发起接龙权限门控）。小程序 AppID `wx8ad6ccfa1b8f040a` 已通过 `miniprogram-ci` 上传开发版本 `1.0.6`，robot `1`，备注为“2026-06-13 修复tabBar重叠(padding兜底)/屏闪(CSS图标)/加退出登录”（前一版 `1.0.5` 为 tabBar fixed/未登录按钮/记住昵称一键登录）；尚未提交微信审核或发布正式版。后端/网页端 `express-knlw-049` 已通过 `deploy:verify`，线上 `/api/health` 200 且 DB OK。
