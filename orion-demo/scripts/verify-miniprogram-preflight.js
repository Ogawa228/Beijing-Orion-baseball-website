...
function checkCoreBusinessSurface() {
  ...
  ['wx:if="{{event}}" class="section card panel stack"', 'event-cover-hero', 'event-gallery', 'previewEventImage', '复制原帖链接', '关联比赛', '从接龙记分', 'linkedGames', '加载更多关联比赛', '管理员手动增补', '搜索姓名 / 背号后选择球员', '加载更多球员', '加入/更新名单', 'data-status="cancelled"', '未建档成员只作为手动姓名', '管理接龙', '编辑接龙', '删除接龙'].forEach(token => {
  ...
  ['bind_requests:review', 'points:write', 'attendances:write', 'tournaments:write', 'games:draft', 'games:revise', 'hof:write', 'highlights:write', 'notifications:write', 'bind_codes:manage', 'users:read', 'users:grant_admin', 'users:password_reset', 'users:app_connect_code', 'users:bind_direct', 'destructive:delete', 'audit:game_read', 'system:settings', "api.get('/admin/bind-requests'", 'bindRequestStatusOptions', 'onBindRequestStatusChange', 'bindRequestStatusLabel', "api.get('/admin/users'", 'ADMIN_USER_PAGE_LIMIT', 'adminUsersHasMore', 'adminUsersNextOffset', 'loadMoreAdminUsers', 'mergeAdminUsersById', 'adminBindFilterOptions', 'onAdminBindFilterChange', 'matchesAdminBindStatus', 'adminUserSortOptions', 'onAdminUserSortChange', 'onAdminUserSearchInput', 'matchesAdminSearch', 'compareAdminUsers', "api.patch(`/admin/users/", "api.post(`/admin/users/${target.id}/app-connect-code", "api.post(`/admin/users/${target.id}/reset-password", "api.post(`/admin/users/${target.id}/bind-player", "api.post(`/admin/users/${target.id}/unbind-player", "api.del(`/admin/users/${target.id}`", "api.get('/tournaments'", "api.post('/tournaments'", "api.patch(`/tournaments/", "api.del(`/tournaments/${selected.id}`", "api.get('/games'", "api.patch('/games/batch-reassign'", 'batchReassignGames', "api.del(`/games/${id}`", 'batchDeleteGames', 'canDeleteGames', "api.get('/admin/bind-invitation-options'", "api.get('/bind-codes'", "api.post('/bind-codes'", "api.del(`/bind-codes/", 'createBindCode', 'deleteBindCode', 'copyBindCode', 'ADMIN_PLAYER_PAGE_LIMIT', 'adminPlayerParams', "api.get('/players', adminPlayerParams", 'playersHasMore', 'playersNextOffset', 'loadMoreAdminPlayers', 'mergePlayersById', 'visiblePlayerPool', 'playerPoolSummary', 'onPlayerPoolSearchInput', 'onPlayerPoolLevelChange', 'openPoolPlayer', 'openPoolHighlights', 'selectPoolPlayerRecords', 'selectPoolMergeSource', 'createPoolBindCode', 'upgradePoolPlayer', "api.post(`/players/${id}/upgrade", 'visibleAttendancePlayers', 'selectedAttendancePlayerIds', 'onAttendancePlayerSelectionChange', 'selectAllAttendancePlayers', 'clearAttendancePlayers', 'pointGameId', 'pointGameOptions', 'onPointGameSearchInput', 'onPointGameChange', 'clearPointGame', 'ADMIN_EVENT_PAGE_LIMIT', "api.get('/events', { limit: ADMIN_EVENT_PAGE_LIMIT", 'eventsHasMore', 'eventsNextOffset', 'loadMoreAdminEvents', 'mergeEventsById', "api.get('/admin/audit-logs'", "api.post('/admin/bind-invitations'", "api.post('/players'", "api.post('/players/import'", "api.post('/upload/base64'", "chooseAndUploadImage('player'", "chooseAndUploadImage('tournament'", 'chooseTournamentCover', 'clearTournamentCover', 'tournamentCoverUploadText', 'wx.chooseMedia', 'wx.getFileSystemManager().readFile', 'chooseCreatePlayerPhoto', 'clearCreatePlayerPhoto', 'createPlayerPhoto', 'chooseBatchPlayerPhotos', 'clearBatchPlayerPhotos', 'batchPlayerPhotos', 'imageFileBaseName', 'createPlayer', 'importPlayersBatch', 'playerCreateLevelOptions', '试训队员', '正式球员', 'player_delete', "api.get('/leaderboard'", 'leaderboardOverview', 'formatLeaderboardOverview', 'openLeaderboardPlayer', "api.post('/points-adjustments'", "api.get('/points-adjustments'", "api.del(`/points-adjustments/", 'deletePointAdjustment', "api.post('/attendances'", "api.get('/attendances'", "api.del(`/attendances/", 'deleteAttendanceRecord', "api.post('/hall-of-fame'", "api.get('/highlights'", "api.del(`/hall-of-fame/", 'delete_game', "api.get('/site-settings/players-starfield'", "api.call('/site-settings/players-starfield'", 'publishStarfieldSettings', 'starfieldPresetOptions'].forEach(token => {
  ...
  ['ADMIN_TOURNAMENT_PAGE_LIMIT', "api.get('/tournaments', { limit: ADMIN_TOURNAMENT_PAGE_LIMIT", "api.get(`/tournaments/${id}`)", 'tournamentHasMore', 'tournamentNextOffset', 'loadMoreAdminTournaments', 'mergeTournamentsById'].forEach(token => {
    assert(adminJs.includes(token), `mini admin tournament pagination missing ${token}`);
  });
  ...
  ['猎户管理台', '星阵全站发布', '发布到全站', '恢复默认', 'players-starfield', '赛事设置', '保存赛事', '赛事封面', '上传封面', '清空封面', 'tournament-cover-preview', '删除赛事', '批量移动比赛', '全选当前', '移动选中比赛', '删除选中比赛', '输入“删除比赛”确认', '账号管理', '搜索昵称、邮箱、球员、号码', '加载更多账号', 'adminBindFilterOptions', 'adminBindFilterLabel', 'adminUserSortLabel', '保存权限', '生成网页关联码', '网页关联码', '重置网页密码', '直接绑定', '解除绑定', '危险操作', '删除账号', '绑定审批', 'bindRequestStatusOptions', 'bindRequestStatusLabel', '发送绑定邀请', '绑定码中心', '生成绑定码', '作废', '最近操作', '审计日志', '注册球员池', '搜索姓名、别名、背号、守位', 'player-pool-row', '档案', '时刻', '升级', '绑码', 'points-panel', '批量出席名单', 'attendance-player-row', 'attendanceSubmitLabel', '关联比赛（可选）', 'pointGameLabel', '新增球员', '真实球员照片', '上传照片', '清空照片', 'create-player-photo-preview', '批量粘贴导入', '批量照片匹配', '选择照片', '导入名单', '全员积分总览', 'leaderboardOverview', 'openLeaderboardPlayer', '手动调整积分', '暂无手动积分记录', '补录签到', '暂无签到记录', '名人堂授予', '加入名人堂', '移出名人堂', '精彩时刻', '数据导入', '比赛 PDF / Excel 可在小程序解析预览后入库'].forEach(token => {
    assert(adminWxml.includes(token), `mini admin UI missing ${token}`);
  });
  ['加载更多赛事', 'loadingMoreTournaments', 'loadMoreAdminTournaments'].forEach(token => {
    assert(adminWxml.includes(token), `mini admin tournament pagination UI missing ${token}`);
  });
  ['队内接龙管理', '删除选中接龙', '输入“删除接龙”确认', 'selectedEventCount', '加载更多接龙', '搜索标题、分类、日期、地点', 'eventTagFilterOptions', 'eventTagFilterLabel', '详情', '编辑'].forEach(token => {
    assert(adminWxml.includes(token), `mini admin event batch UI missing ${token}`);
  });
  ...
  ['赛事 / 赛季', 'tournament-card', 'bindtap="setTournament"', 'catchtap="openTournament"', '详情', '{{item.countText}}', 'tournamentHasMore', 'loadMoreTournaments', '加载更多赛事', 'bindtap="loadMore"', '加载更多比赛', '{{item.tournamentLabel', 'game-cover', 'game-cover-img', '{{item.cover}}'].forEach(token => {
  ...
  ['精彩时刻', 'highlight-grid', '复制B站链接', '加载更多精彩时刻', 'loadingMoreHighlights', 'bindtap="previewHighlight"', 'bindtap="copyHighlightLink"', '{{item.metaClass}}', 'log-link {{item.nameClass}}'].forEach(token => {
  ...
  ['基础信息', '阵容来源', '阵容工作台', 'roster-list', 'lineup-workbench', '出场名单来源', '接龙活动', '加载更多接龙候选', 'loadingMoreRosterEvents', '加载更多赛事候选', 'loadingMoreTournaments', '搜索姓名 / 背号 / 守位', '加载更多球员', 'loadingMorePlayers', '管理员可以在接龙导入后继续勾选', 'catchtouchmove', 'onFieldMarkerTap', 'moveLineupUp', 'moveLineupDown'].forEach(token => {
  ...
  ['比赛局面', '事件日志', '换半局 / 下一局', '垒上跑者', '手动打点 RBI', '直接选择当前打者', 'selectBatterRow', '阵容调整', '记录阵容调整'].forEach(token => {
  ...
  ['PLAYER_LINK_QUERY_LIMIT', 'collectGamePlayerRefs', 'loadPlayersForGame', "api.get('/players', { include: 'all', keyword: name, limit: PLAYER_LINK_QUERY_LIMIT, offset: 0 })", 'buildPlayerIndexes', 'linkRows', 'linkMvp', 'gameOrigin', 'openPlayer', 'openEvent', 'exportGamePdf', '/export-pdf', 'wx.openDocument', 'writeFile', 'player.publicDisplayName', 'BAT_SORT_OPTIONS', 'PITCH_SORT_OPTIONS', 'applyTableSort', 'decorateBattingRows', 'decoratePitchingRows', 'chartRows', 'oppPitching'].forEach(token => {
...
function checkMiniProgramUiGuards() {
  ...
  ['wx.switchTab', 'setActiveByRoute', '活动中心', '数据中心', '签到', '积分榜', '我的'].forEach(token => {
  ...
  ['api.patch(`/events/${this.data.id}`', "api.post('/upload/base64'", "kind: 'event'", 'chooseCoverImage', 'chooseGalleryImages', 'removeGalleryImage', 'previewGalleryImage', 'sourceLink', 'wx.chooseLocation', 'chooseEventLocation', 'buildLocationMetadata', 'parseXhsPaste', 'pasteAndParseXhs', 'parseXhsContent', 'wx.getClipboardData', 'wx.chooseMedia', 'wx.getFileSystemManager().readFile', 'images: normalizeImages'].forEach(token => {
    assert(eventCreateJs.includes(token), `event create edit/cover behavior missing ${token}`);
  });
  ['一键导入帖子内容', '识别并填充', '粘贴识别', 'xhsRaw', '接龙封面图', '上传封面', '接龙配图', '上传配图', 'removeGalleryImage', '原帖链接', '地图选择', 'locationMetaText', '{{submitLabel}}'].forEach(token => {
  ...
  assert(eventListWxml.includes('event-list-head') && eventListWxml.includes('action-btn'), 'event list head should use narrow-safe classes');
  ...
  ['EVENT_PAGE_LIMIT', 'hasMore', 'nextOffset', 'loadMore', 'mergeEventsById', "'/auth/me'"].forEach(token => {
  ...
  assert(eventListWxss.includes('display: flex') && eventListWxss.includes('width: 0'), 'event list actions should fit within page width');
