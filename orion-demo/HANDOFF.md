...
- 当前状态：线上为 `express-knlw-049`（2026-06-12 修复绑定申请微信登录身份显示、核验信息口径和小程序发起接龙权限门控）。小程序 AppID `wx8ad6ccfa1b8f040a` 已通过 `miniprogram-ci` 上传开发版本 `1.0.5`，robot `1`，备注为“2026-06-13 修复tabBar重叠/频闪、未登录按钮、登录记住昵称二次一键登录”（前一版 `1.0.4` 为 tabBar 矢量图标/频闪/个人面板/管理台溢出精修）；尚未提交微信审核或发布正式版。后端/网页端 `express-knlw-049` 已通过 `deploy:verify`，线上 `/api/health` 200 且 DB OK。
