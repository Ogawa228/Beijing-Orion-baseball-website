...
- 当前状态：线上后端为 `express-knlw-057`（本轮纯小程序改动,后端无变化,按需未重发）。小程序 AppID `wx8ad6ccfa1b8f040a` 已通过 `miniprogram-ci` 上传开发版本 `1.0.14`，robot `1`，备注为“2026-06-14 图片上传统一高可用工具(压缩+uploadFile直传+callContainer兜底)”；尚未提交微信审核或发布正式版。后端/网页端 `express-knlw-057` 仍 `normal`，线上 `/api/health` 200 且 DB OK。
- ⚠️ 图片上传高可用:新增 `miniprogram/utils/upload.js` 作为唯一图片上传入口(8 个页面统一接入),压缩(1280/q78 对齐网页)→ `wx.uploadFile` 直传 `/api/upload`(主,绕过 callContainer 大小限制)→ callContainer base64 兜底。**要让主通道生效,需在微信公众平台「开发管理-开发设置-服务器域名」的 `uploadFile 合法域名` 加 `https://express-knlw-255356-7-1429688831.sh.run.tcloudbase.com`**;未配置时自动降级到 callContainer 兜底(压缩后通常也能过),功能不受影响,只是少了主通道。
...
2026-06-14（十二）图片上传统一为高可用工具（仅小程序改动,已上传开发版 `1.0.14`,robot `1`；后端无改动按需未重发,线上仍 `express-knlw-057`；尚未提交审核或发布）：用户要求"彻底解决图片上传,对齐网页端质量,务必高可用,不再出 cloud callContainer BUG"。① 根因 = 全部图片上传走 `wx.cloud.callContainer` 把整图 base64 当 JSON data 传,callContainer 请求体有大小限制/易超时,大图直接报 cloud callContainer 错误。② 新建 `miniprogram/utils/upload.js` 作为唯一图片上传入口:`uploadImage(file, kind, prefix)` = `wx.compressImage`(限宽 1280 + 质量 78,对齐网页端 1280/q0.78,带旧基础库 fallback)→ 主通道 `wx.uploadFile` multipart 直传 `/api/upload`(带 Bearer 鉴权,绕过 callContainer 大小限制,大图稳定)→ 兜底 `callContainer` base64 传 `/api/upload/base64`(uploadFile 合法域名未配/网络异常时自动降级,压缩后体积小通常也能过);两通道都失败才抛错。③ 8 个上传页面(tournament-manage / event-create 封面+配图 / profile / player-detail / admin / highlights / game-edit / game-import 封面)全部改为统一调 `upload.uploadImage`,删除各自重复的压缩/读 base64/post 上传代码;game-import 的 PDF/Excel 数据导入仍走原 `import-gamechanger`,不动。④ 后端 `/api/upload`(multipart,multer,20MB)和 `/api/upload/base64`(12MB)都已存在且返回 `{url}` 格式一致,无需改后端。⑤ 测试:`test:miniprogram-flows` 的 `loadPage` 清 `utils/upload` 缓存使其重新绑定当前 mock api(util 内部 `require('./request')`),上传测试走兜底 base64 通道断言 payload;preflight 把各页面 `api.post('/upload/base64')`、`kind:'xxx'` 断言改为 `upload.uploadImage` / `'kind', 'prefix'`。完整 `npm run test:miniprogram-ci` 17 项全绿、`node --check`、`git diff --check` 通过。**部署提示**:要启用 uploadFile 主通道,需在微信后台配 uploadFile 合法域名(见上方接手摘要);不配也有 callContainer 兜底保底。

2026-06-14（十一）赛事封面上传修复与地点纯地图选点（已部署后端 `express-knlw-057`；已上传小程序开发版 `1.0.13`，robot `1`；尚未提交审核或发布）：修复用户反馈两个问题（纯小程序前端改动,无后端逻辑变化,后端按全量部署惯例重发 057,代码同 056）。① 赛事封面上传失败(报 cloud callContainer 错误)根因 = 图片上传走 `wx.cloud.callContainer`,而 callContainer 请求体有大小限制(也可能超时),原图 base64 过大上传失败。修复:上传前用 `wx.compressImage`(`quality:65 + compressedWidth:1280`,旧基础库不支持 compressedWidth 时退回只压质量,再失败用原图)压缩,把 base64 体积压进限制内;`tournament-manage` 的 `uploadImageFile` 和 `event-create` 的 `readFileBase64`(封面 + 配图共用)都加了压缩,profile/admin 头像图较小暂未改。不改 callContainer 架构(callContainer 不需配域名白名单,改 HTTP uploadFile 需用户在微信后台配合法域名)。若压缩后仍偶发失败,下一步可让用户配 uploadFile 合法域名改 `wx.uploadFile` 直传 `/api/upload`(multipart)。② 赛事地点删除文字输入,改纯地图选点:`tournament-manage` 地点字段从 `input` + onLocationInput 改为 `picker-like` 只读展示 + 点击触发 `wx.chooseLocation`(显示已选地点名,未选显示"点击地图选择地点"灰字),与签到地理围栏需要坐标的设计一致;删除 `onLocationInput` handler。`test:miniprogram-flows` 的赛事创建用例把 `onLocationInput` 改为 `setData({location,locationLatitude,locationLongitude})` 模拟选点结果。验证:完整 `npm run test:miniprogram-ci` 17 项全绿、`node --check`、`git diff --check`;`wxcloud run:deploy` 发布 `express-knlw-057`、`deploy:verify --expected-version express-knlw-057` 通过(四项 normal/ok 全 true);小程序开发版 `1.0.13` 上传成功。
