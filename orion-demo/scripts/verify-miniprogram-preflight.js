...
function checkCoreBusinessSurface() {
  ...
  ['tournaments:write', 'games:revise', 'destructive:delete', "api.get(`/tournaments/${this.data.id}`)", "api.post('/tournaments'", "api.patch(`/tournaments/${this.data.id}`", "api.del(`/tournaments/${this.data.id}`", "api.patch('/games/batch-reassign'", "api.get('/games'", 'upload.uploadImage', "uploadImageFile(file, 'tournament'", 'chooseCoverImage', 'moveSelectedGames', 'deleteConfirmText', '删除赛事'].forEach(token => {
  ...
  ["api.get(`/players/${this.data.id}`)", "api.get(`/players/${this.data.id}/points`)", "api.get('/games'", 'PLAYER_GAME_PAGE_LIMIT', 'playerGameParams', 'gamesHasMore', 'gamesNextOffset', 'loadMorePlayerGames', 'mergeGamesById', "api.get('/highlights'", "public: 'true'", 'playerName', 'limit: 24', 'fetchPlayerPublicHighlights', 'playerHighlightNames', 'playerIdentity', 'nameClass', 'avatarClass', 'relatedGames', 'capabilityProfile', 'capabilityRosterFromGames', 'playerHighlights', 'previewHighlight', 'copyHighlightLink', 'submitPlayerHighlight', 'wx.previewImage', 'wx.setClipboardData', 'rowMatches', 'openGame', 'openTimelineItem', 'timelineTarget', '/pages/events/event-detail/event-detail', "api.patch(`/players/${this.data.id}`", "api.patch(`/players/${this.data.id}/public-profile`", 'upload.uploadImage', "api.post(`/players/${this.data.id}/upgrade`", "api.del(`/players/${this.data.id}`", 'canEditPlayer', 'canDeletePlayer', 'destructive:delete', 'deleteConfirmText', 'deletePlayer', 'onDeleteConfirmInput', 'editPhoto', 'editAliases', 'editPublicDisplayName', 'choosePlayerPhoto', 'clearPlayerPhoto', 'choosePublicAvatar', 'wx.chooseMedia', 'wx.getFileSystemManager().readFile'].forEach(token => {
  ...
  ["api.get(`/games/${this.data.id}`)", "api.get('/tournaments', {", 'TOURNAMENT_PAGE_LIMIT', 'tournamentHasMore', 'tournamentNextOffset', 'loadMoreTournaments', 'mergeTournamentsById', 'normalizeNextOffset', "api.get(`/tournaments/${currentTournamentId}`)", "api.patch(`/games/${this.data.id}`", "api.del(`/games/${this.data.id}`", 'upload.uploadImage', 'games:revise', 'destructive:delete', '_revisionReason', "mini_game_edit", 'normalizeBattingRow', 'normalizePitchingRow', 'chooseCoverImage', 'deleteGame', 'deleteConfirmText', "'game', 'game-cover'", 'wx.chooseMedia', 'wx.getFileSystemManager().readFile', 'cover: String(this.data.cover'].forEach(token => {
  ...
  ["wx.chooseMessageFile", "extension: ['pdf', 'xls', 'xlsx']", "wx.getFileSystemManager().readFile", "api.get('/tournaments', {", 'TOURNAMENT_PAGE_LIMIT', 'tournamentHasMore', 'tournamentNextOffset', 'loadMoreTournaments', 'mergeTournamentsById', 'upload.uploadImage', "'game', 'game-cover'", "api.post('/games/import-gamechanger'", "api.post('/games'", 'games:draft', 'games:confirm', 'chooseCoverImage', 'coverUrl', 'expandImportFileDrafts', 'parseSelectedFile'].forEach(token => {
  ...
  ["api.patch('/auth/me'", "api.patch(`/players/${player.id}/public-profile`", 'canEditPublicProfile', 'publicPreviewName', 'players:display_write', 'upload.uploadImage', 'chooseAccountAvatar', 'choosePublicAvatar', 'wx.chooseMedia', 'wx.getFileSystemManager().readFile'].forEach(token => {
