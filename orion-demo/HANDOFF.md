...
- 当前状态：线上为 `express-knlw-057`（2026-06-14 赛事封面压缩修上传失败 + 赛事地点改纯地图选点；纯小程序改动,后端代码与 `056` 一致,按全量部署惯例重发）。小程序 AppID `wx8ad6ccfa1b8f040a` 已通过 `miniprogram-ci` 上传开发版本 `1.0.13`，robot `1`，备注为“2026-06-14 赛事封面压缩修上传失败、地点改纯地图选点”；尚未提交微信审核或发布正式版。后端/网页端 `express-knlw-057` 已通过 `deploy:verify`，线上 `/api/health` 200 且 DB OK。
